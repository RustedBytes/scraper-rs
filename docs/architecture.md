...
`scraper_rs` is a Python extension module implemented in Rust with PyO3. The Rust module entry point is `src/lib.rs`, while the public API is split across focused modules such as `src/document.rs`, `src/element.rs`, and `src/functions.rs`. The Python package in `scraper_rs/__init__.py` explicitly re-exports the supported extension-module API.
...
2. `Document::parse_with_limit` enforces size limits via `ensure_within_size_limit` in `src/limits.rs` (with optional truncation).
...
Key code references:
- Document parsing: `src/document.rs` (`Document::parse_with_limit`, lazy XPath state)
- Size limits: `src/limits.rs` (`DEFAULT_MAX_PARSE_BYTES`, `ensure_within_size_limit`)
- CSS selection and selector cache: `src/selectors.rs` (`SELECTOR_CACHE`, `parse_selector`) and `src/element.rs` (`snapshot_element`)
- XPath parsing, cache, selection, and conversion: `src/xpath.rs`
- Top-level helper functions: `src/functions.rs`
...
- Top-level async functions (`select`, `xpath`, `select_first`, `first`, etc) yield once and then call the corresponding synchronous Rust helper from `src/functions.rs`.
...
- Rust sync entry points: `src/document.rs`, `src/element.rs`, and `src/functions.rs`

## Module wiring

The Rust module initializer in `src/lib.rs` registers classes and functions with the Python extension module:

- `#[pymodule] fn scraper_rs(...)` adds `Document`, `Element`, parse-tree helpers, prettify helpers, selector helpers, and extension-level async helpers.
- The version string is exposed as `__version__` from `env!("CARGO_PKG_VERSION")`.
- `scraper_rs/__init__.py` re-exports the supported sync API explicitly; `scraper_rs/asyncio.py` provides the coroutine-facing API.
...
- Async examples: `examples/demo_asyncio.py`, `examples/demo_async_document.py`
